...
class MyTvTelenorHU(CBaseHostClass):
        ...
        try:
            ...
            if not link or expires < time.time():
                ...
                if not med:
                    ...
                    for i in meds:
                        if i["StreamingType"] != "HLS":
                            continue
                        if not med and i["Quality"] == "SD" or i["Quality"] == "HD":
                            med = i["Pid"]
                    if not med:
                        return videoUrls
                    cItem["med"] = med
    ...
    def tryTologin(self):
        ...
        if not needLogin and self.token and self.tokenExpires > time.time():
            return
        ...
        try:
            ...
            if not emptyLogin:
                ...
                if not needLogin:
                    ...
                    if sts:
                        data = json_loads(data)
                    elif self.cm.meta.get('status_code') == 401:
                        needLogin = True
                    else:
