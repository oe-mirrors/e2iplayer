...
#            httpParams['raw_post_data'] = True
            ...
            if sts and 'success' in data:
                ...
                if sts:
                    message = self.cm.ph.getSearchGroups(data, '''['"]error['"][^>]*?['"]([^'^"]+?)['"]''')[0]
                else:
                    message = ''
    ...
    def getArticleContent(self, cItem):
        ...
        if title == '':
            title = self.cm.ph.getDataBeetwenMarkers(data, '<h1>', '</h1>', True)[1]
#        icon = self.getFullIconUrl(self.cm.ph.getSearchGroups(title, '''this\.src=['"]([^"^']+?)['"]''', 1, True)[0])
        ...
        if '.. więcej</a>' in desc:
            desc = desc.replace('.. więcej</a>', '</a>')
        if desc == '':
            desc = self.cm.ph.getDataBeetwenMarkers(data, '<div class="clearfix"><img', '</div>', True)[1]
