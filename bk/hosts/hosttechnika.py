...
class Technika(CBaseHostClass):
    ...
    def rpttl(self, title='', tpe=''):
        ...
        try:
            if title != '' and tpe != '':
                if tpe == '30':
                    bv = title.replace('Tech2 - ', '').strip()
                if tpe == '31':
                    bv = title.replace('PowerTech - ', '').strip()
                if tpe == '32':
                    bv = title.replace('Mobeel - ', '').strip()
                if tpe == '33':
                    bv = title.replace('ITFroccs - ', '').strip()
                if tpe == '34':
                    bv = title.replace('TechVideoHU - ', '').strip()
                if tpe == '35':
                    bv = title.replace('Hasznalt Droid - ', '').strip()
                if tpe == '36':
                    bv = title.replace('TheVR Tech - ', '').strip()
                if tpe == '37':
                    bv = title.replace('Notebook.hu - ', '').strip()
                if tpe == '38':
                    bv = title.replace('Apple Pie - ', '').strip()
                if tpe == '39':
                    bv = title.replace('ShadowWarrior - ', '').strip()
                if tpe == '40':
                    bv = title.replace('Hunboxing - ', '').strip()
                if tpe == '41':
                    bv = title.replace('ByTech - ', '').strip()
                if tpe == '42':
                    bv = title.replace('TechKalauz - ', '').strip()
                if tpe == '43':
                    bv = title.replace('TechDoboz - ', '').strip()
                if tpe == '44':
                    bv = title.replace('AnonimInc - ', '').strip()
                if tpe == '45':
                    bv = title.replace('PCX - ', '').strip()
                if tpe == '46':
                    bv = title.replace('BiTech - ', '').strip()
                if tpe == '47':
                    bv = title.replace('The HUB - ', '').strip()
                if tpe == '48':
                    bv = title.replace('RPs Lair - ', '').strip()
                if tpe == '49':
                    bv = title.replace('TechFunDo - ', '').strip()
            ...
            if baseUrl != '':
                ...
                try:
                    ...
                    for item in data:
                        ...
                        if mk == 'tech2':
                            tempdesc = self.szovegki_tech2(tempdesc)
                        if mk == 'powertech':
                            tempdesc = self.szovegki_powertech(tempdesc)
                        if mk == 'mobeel':
                            tempdesc = self.szovegki_mobeel(tempdesc)
                        if mk == 'itfrocs':
                            tempdesc = self.szovegki_itfrocs(tempdesc)
                        if mk == 'techvideo':
                            tempdesc = self.szovegki_techvideo(tempdesc)
                        if mk == 'hdroid':
                            tempdesc = self.szovegki_hdroid(tempdesc)
                        if mk == 'vrtech':
                            tempdesc = self.szovegki_vrtech(tempdesc)
                        if mk == 'nbook':
                            tempdesc = self.szovegki_nbook(tempdesc)
                        if mk == 'apie':
                            tempdesc = self.szovegki_apie(tempdesc)
                        if mk == 'shadowwarrior':
                            tempdesc = self.szovegki_shadowwarrior(tempdesc)
                        if mk == 'hunboxing':
                            tempdesc = self.szovegki_hunboxing(tempdesc)
                        if mk == 'bytech':
                            tempdesc = self.szovegki_bytech(tempdesc)
                        if mk == 'techkalauz':
                            tempdesc = self.szovegki_techkalauz(tempdesc)
                        if mk == 'techdoboz':
                            tempdesc = self.szovegki_techdoboz(tempdesc)
                        if mk == 'anonim':
                            tempdesc = self.szovegki_anonim(tempdesc)
                        if mk == 'pcx':
                            tempdesc = self.szovegki_pcx(tempdesc)
                        if mk == 'bitech':
                            tempdesc = self.szovegki_bitech(tempdesc)
                        if mk == 'thehub':
                            tempdesc = self.szovegki_thehub(tempdesc)
                        if mk == 'rplair':
                            tempdesc = self.szovegki_rplair(tempdesc)
                        if mk == 'techfundo':
                            tempdesc = self.szovegki_techfundo(tempdesc)
    ...
    def susmrgts(self, i_md='', i_hgk='', i_mptip='', i_mptipe='', i_mpu='', i_mpt='', i_mpi='', i_mpdl='', i_mpnzs='', i_mpyd=''):
        ...
        try:
            ...
            if i_mpyd != '':
                i_mpyd = base64.b64encode(i_mpyd).replace('\n', '').strip()
                ...
                for temp_item in data:
                    ...
                    for item in temp_data:
                        ...
                        if t_vp == 'c_sor_tipe':
                            ...
                            if temp_tpe != '':
                                temp_tpe = base64.b64decode(temp_tpe)
                        if t_vp == 'c_sor_yd':
        ...
        try:
            if i_md != '' and i_hgk != '' and i_mptip != '' and i_mpks != '':
                ...
                if i_mpdb != '':
                    i_mpdb = base64.b64encode(i_mpdb).replace('\n', '').strip()
                if i_mpks != '':
                    i_mpks = base64.b64encode(i_mpks).replace('\n', '').strip()
        ...
        try:
            if i_md != '' and i_hgk != '' and i_mptipe != '':
                ...
                if i_mptipe != '':
                    i_mptipe = base64.b64encode(i_mptipe).replace('\n', '').strip()
                if i_mpno != '':
                    i_mpno = base64.b64encode(i_mpno).replace('\n', '').strip()
