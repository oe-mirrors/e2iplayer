...
class StreamZenNet(CBaseHostClass):
    ...
    def listItems(self, cItem):
        ...
        try:
            ...
            for items in block:
                if '/?=' in tmpurl:
                    title = self.cleanHtmlStr(self.cm.ph.getAllItemsBeetwenNodes(items, '<h3 class="Title">', '</h3>', False)[0])
                else:
                    title = self.cleanHtmlStr(self.cm.ph.getAllItemsBeetwenNodes(items, '<div class="Title">', '</div>', False)[0])
                ...
                if '<span class="Qlty">' in items:
                    quality = '   \c00????00[' + self.cm.ph.getAllItemsBeetwenNodes(items, '<span class="Qlty">', '</span>', False)[0] + ']'
                else:
                    quality = ''
                title += quality
                params = dict(cItem)
    ...
    def handleService(self, index, refresh=0, searchPattern='', searchType=''):
        ...
        elif category == 'byyears':
            self.buildYears(self.currItem)
        elif category == 'bygenres':
            self.buildGenre(self.currItem)
        elif category == 'listyears' or category == 'listgenres':
            self.listItems(self.currItem)
