...
from Plugins.Extensions.IPTVPlayer.tools.iptvtools import byteify, printDBG, printExc, rm
...
class MaxtvGO(CBaseHostClass):
    def __init__(self):
        ...
        self.MAIN_CAT_TAB = [
            {"category": "list_items", "title": "MaxTVGo", "url": self.getFullUrl("/api/videos.php?action=find")},
    ...
    def listItems(self, cItem, nextCategory):
        ...
        try:
            ...
            printExc()
            if self.loggedIn is not True and len(self.currList) == 0:
    ...
    def getLinksForVideo(self, cItem):
        ...
        if self.up.checkHostSupport(cItem.get("url", "")) == 1:
        ...
        for item in tmp:
            ...
            if not self.cm.isValidUrl(url):
                continue
            typ = self.getFullUrl(self.cm.ph.getSearchGroups(item, """type=['"]([^'^"]+?)['"]""")[0]).lower()
            if "mp4" in typ:
    ...
    def tryTologin(self):
        ...
        if None is self.loggedIn or self.login != config.plugins.iptvplayer.maxtvgo_login.value or self.password != config.plugins.iptvplayer.maxtvgo_password.value:
