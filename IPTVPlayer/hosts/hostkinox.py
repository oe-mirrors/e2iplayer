...
from Plugins.Extensions.IPTVPlayer.tools.iptvtools import byteify, printDBG, printExc
...
def cap(html):
    ids = re.findall(r'data-id="([^"]+)" data-value="[^>]+ title="([^"]+)', html)
    text_match = re.search(r"<h3>([^<]+)", html)
    p1 = re.findall(r'text" name="([^"]+)', html)
    p2 = re.findall(r'name="([^"]+)" value="([^"]+)', html)
    p3 = re.findall(r'submit" name="([^"]+)', html)
    p4 = re.findall(r'hidden" name="([^"]+)" id', html)
    if all([ids, text_match, p1, p2, p3, p4]):
        allowed = {v for k, v in ids}
        text_match = text_match.group(1)
        words = re.findall(r"\b\w+\b", text_match)
...
class Kinox(CBaseHostClass):
    ...
    def fillCacheFilters(self, cItem):
        ...
        def addFilter(data, marker, baseKey, addAll=True):
            ...
            if len(self.cacheFilters[key]):
                if addAll:
                    self.cacheFilters[key].insert(0, {"title": _t("All")})
                self.cacheFiltersKeys.append(key)
    ...
    def listNewTab(self, cItem, nextCategory):
        ...
        for tab in data:
            ...
            for item in tab:
                ...
                desc = _t("Language") + ": " + langsMap.get(langId, _t("Unknown")) + " | " + _t("Rating") + ": " + self.cleanHtmlStr(item[5])
    ...
    def listFilters(self, cItem, nextCategory):
        ...
        if len(self.cacheFiltersKeys) == 0:
            return
        filtern = self.cacheFiltersKeys[f_idx]
        f_idx += 1
        cItem["f_idx"] = f_idx
        if f_idx == len(self.cacheFiltersKeys):
            cItem["category"] = nextCategory
        self.listsTab(self.cacheFilters.get(filtern, []), cItem)

    def getLangFlags(self):
        if len(self.cacheLangFlags.get("list", [])) == 0:
            langList = []
            langMap = {}
    ...
    def listItems(self, cItem, nextCategory):
        ...
        for item in data:
            ...
            langId = self.cm.ph.getSearchGroups(item, r"""/lng/([0-9]+?)\.png""")[0]
            desc = _t("Language") + ": " + langsMap.get(langId, _t("Unknown")) + " | " + self.cleanHtmlStr(self.cm.ph.getDataBeetwenReMarkers(item, re.compile('<div[^>]*?class="Genre"[^>]*?>'), re.compile('<div class="clearboth">'))[1].replace('<div class="floatright">', "|")).replace(" , ", ", ")
        ...
        if nextPage:
            params = dict(cItem)
            params.update({"title": _t("Next page"), "page": page + 1})
            self.addDir(params)
    ...
    def getVideoLinks(self, videoUrl):
        ...
        if not sts:
            printDBG("Kinox: Fehler beim Abrufen von %s" % videoUrl)
            return []
        js = json.loads(data)
        match = re.search(r'src="([^"]+)', js.get("Stream"))
        if match:
            url2 = "https://www22.kinox.to" + match.group(1)
            sts, html = self.getPage(url2, params)
            if not sts:
                return []
            if "ust reload" in html:
                sts, html = self.getPage(url2, params)
                if not sts:
                    return []
        GetIPTVSleep().Sleep(2)
        von = re.search(r"von (\d)", html)
    ...
    def handleService(self, index, refresh=0, searchPattern="", searchType=""):
        ...
        if category == "news":
            ...
            self.listsHistory({"name": "history", "category": "search"}, "desc", _t("Type: "))
        else:
            printExc()
...
class IPTVHost(CHostBase):
    ...
    def withArticleContent(self, cItem):
        return cItem["type"] == "video" or cItem.get("category") in ["explore_item", "list_episodes"]
