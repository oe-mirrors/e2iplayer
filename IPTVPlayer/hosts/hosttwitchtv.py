...
class Twitch(CBaseHostClass):
    ...
    def listChannel(self, cItem):
        ...
        post_data = []
        post_data.append('{"operationName":"ChannelRoot_Channel","variables":{"currentChannelLogin":"%s","includeChanlets": true},"extensions":{"persistedQuery":{"version":1,"sha256Hash":"90a23e279e3ed997efcf53550abc3570bbaabee73f78d2f36bef770fa232e91b"}}}' % login)
        ...
        try:
            ...
            try:
                if data[0]['data']['user']['stream']['type'] == 'live':
                    descTab = []
                    viewers = str(data[0]['data']['user']['stream']['viewersCount'])
                    descTab.append(_('%s viewers') % viewers)
                    item = data[0]['data']['user']['lastBroadcast']
