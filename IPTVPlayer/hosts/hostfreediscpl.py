...
class FreeDiscPL(CBaseHostClass):
    ...
    DEFAULT_ICON_URL = "https://i.imgur.com/mANjWqL.png"
    MAIN_CAT_TAB = [{"category": "list_filters", "title": "Najnowsze publiczne pliki użytkowników", "url": MAIN_URL + "explore/start/get_tabs_pages_data/%s/newest/"}, {"category": "list_filters", "title": "Ostatnio przeglądane pliki", "url": MAIN_URL + "explore/start/get_tabs_pages_data/%s/visited/"}]
    FILTERS_TAB = [{"title": _("Movies"), "filter": "movies"}, {"title": _("Music"), "filter": "music"}]
    ...
    def tryTologin(self):
        ...
        errMsg = []
        if None is self.loggedIn or self.login != config.plugins.iptvplayer.freediscpl_login.value or self.password != config.plugins.iptvplayer.freediscpl_password.value:
