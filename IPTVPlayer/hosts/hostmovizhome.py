...
class MovizHome(CBaseHostClass):
    def __init__(self):
        CBaseHostClass.__init__(
            self, {"history": "movizhome", "cookie": "movizhome.cookie"}
        )
        ...
        self.DEFAULT_ICON_URL = self.getFullUrl(
            "https://movizhome.click/wp-content/uploads/2024/12/MovizHome-LoGo-238x238.png"
        )
        self.cacheLinks = {}
    ...
    def listMainMenu(self):
        ...
        self.addDir(
            {
                ...
                "category": "list_items",
                "title": "مضاف حديثا",
                "url": self.getFullUrl("/recent/"),
            }
        )
        menu_data = {
            "افلام اجنبيه": [
                "category/افلام-movis/افلام-اجنبية",
                [
                    ("الأكشن", "genre/اكشن"),
                    ("الرعب", "genre/رعب"),
                    ("الخيال العلمي", "genre/خيال-علمي"),
                    ("الفانتازيا", "genre/fantasy-movies"),
                    ("رومانسية", "genre/رومانسية"),
                    ("الكوميديا", "genre/كوميديا"),
                    ("الإثارة والتشويق", "genre/إثارة"),
                    ("الجريمة", "genre/جريمة"),
                    ("المغامرات", "genre/مغامرات"),
                    ("الدراما", "genre/دراما"),
                    ("كلاسيكية", "tag/افلام-كلاسيكية"),
                    ("الغموض", "genre/غموض"),
                    ("عائلية", "genre/عائلي"),
                    ("تاريخية", "genre/تاريخي"),
                    ("الويسترن", "genre/ويسترن"),
                ],
            ],
            "افلام هندية": [
                "category/افلام-movis/افلام-هنديه",
                [
                    ("التيلجو", "tag/telugu"),
                    ("تاميلية", "tag/افلام-تاميلية"),
                    ("ماليالامية", "tag/افلام-ماليالامية"),
                    ("كنادية", "tag/افلام-كنادية"),
                    ("بنجابية", "tag/punjabi"),
                    ("ماراثية", "tag/افلام-ماراثية"),
                    ("بنغالية", "tag/افلام-بنغالية"),
                    ("كجراتية", "tag/افلام-كجراتية"),
                ],
            ],
            "افلام آسيوية": [
                "category/افلام-movis/افلام-اسيويه",
                [
                    ("كورية", "language/الكورية"),
                    ("يابانية", "language/اليابانية"),
                    ("صينية", "language/الصينية"),
                    ("تايلاندية", "language/التايلاندية"),
                    ("إندونيسية", "language/الاندونيسية"),
                    ("فلبينية", "language/الفلبينية"),
                    ("منغولية", "language/المغولية"),
                ],
            ],
        ...
        for title, (main_path, subs) in menu_data.items():
            sub_items = [
                {"title": "افلام %s" % s[0], "url": "%s/%s" % (base, s[1])}
                for s in subs
            ]
            self.addDir(
                {
                    "name": "category",
                    "category": "list_sub_menu",
                    "title": title,
                    "url": "%s/%s" % (base, main_path),
                    "subs": sub_items,
                    "good_for_fav": True,
                }
            )
    ...
    def getFullIconUrl(self, url):
        ...
        return strwithmeta(
            url, {"Cookie": cookieHeader, "User-Agent": self.HEADER.get("User-Agent")}
        )
    ...
    def listItems(self, cItem, nextCategory=None):
        ...
        data = self.cm.ph.getAllItemsBeetwenMarkers(
            htm, '<div class="Small--Box">', "</a>", withMarkers=True
        )
        if not data:
            return
        for item in data:
            try:
                url = self.getFullUrl(
                    self.cm.ph.getSearchGroups(item, '<a[^>]+href="([^"]+)"')[0]
                )
            except Exception:
                continue
            try:
            ...
            try:
                icon = self.getFullIconUrl(
                    self.cm.ph.getSearchGroups(item, 'data-src="([^"]+)"')[0]
                )
            except Exception:
                icon = ""
            genre = self.cm.ph.getSearchGroups(
                item, r'<li[^>]*class="genre"[^>]*>\s*([^<]+)'
            )
            quality = self.cm.ph.getSearchGroups(
                item, r'<li[^>]*class="quality"[^>]*>\s*([^<]+)'
            )
            ...
            if imdb:
                try:
                    ...
                    imdb_color = W
                desc_lines.append(
                    "%sIMDB Rating:%s %s%s%s" % (Y, W, imdb_color, imdb, W)
                )
            desc = "\n".join(desc_lines)
            params = dict(cItem)
            params.update(
        ...
        nextPage = self.cm.ph.getSearchGroups(
            htm, '<a[^>]+class="next page-numbers"[^>]+href="([^"]+)"'
        )
        if nextPage:
            params = dict(cItem)
            params.update(
                {
                    "title": "%s%s%s" % (Y, ("Next Page") + " ▶▶▶", W),
                    "url": self.getFullUrl(nextPage[0]),
                    "good_for_fav": False,
                    "category": "list_items",
                }
            )
    ...
    def listSearchResult(self, cItem, searchPattern, searchType):
        addParams = {
            "header": {"User-Agent": self.cm.getDefaultHeader()["User-Agent"]},
            "use_cookie": True,
            "load_cookie": True,
            "save_cookie": True,
            "cookiefile": self.COOKIE_FILE,
        ...
        sts, data = self.getPage(
            "https://movizhome.click/?s=%s" % searchPattern, addParams
        )
        if not sts:
            return
    ...
    def getLinksForVideo(self, cItem):
        ...
        if not sts:
            return []
        servers = re.findall(
            r'<li[^>]+data-watch="([^"]+)"[^>]*>(.*?)</li>', data, re.S
        )
        for link, item_html in servers:
            name = self.cleanHtmlStr(item_html).strip()
            if not name:
                try:
                    domain = link.split("/")[2]
    ...
    def getArticleContent(self, cItem):
        ...
        genre_li = re.findall(
            r'<li>\s*<i class="fa fa-play"></i>[\s\S]*?<span>نوع الفيلم : </span>([\s\S]*?)</li>',
            data,
            re.S,
        )
        genre_list = []
        ...
        lang_list = re.findall(
            r'<li>\s*<i class="fa fa-language"></i>[\s\S]*?<span>لغة الفيلم : </span>([\s\S]*?)</li>',
            data,
            re.S,
        )
        if lang_list:
        ...
        date = re.findall(
            r"تاريخ اصدار الفيلم : </span>[\s\S]*?<a[^>]+>([^<]+)</a>", data
        )
        date_str = date[0] if date else ""
        user_age = re.findall(
            r"التصنيف العمرى الفيلم : </span>[\s\S]*?<a[^>]+>([^<]+)</a>", data
        )
        user_age_str = user_age[0] if user_age else ""
        imdb = re.findall(
            r'<div class="imdbR"><i class="fa fa-star"></i><span>([\d\.]+)</span>', data
        )
        ...
        if extra_desc:
            full_desc += "\n\n" + extra_desc
        icon_match = re.findall(
            r'<div class="image">[\s\S]*?<img[^>]+src="([^"]+)"', data
        )
        icon = (
            self.getFullIconUrl(icon_match[0]) if icon_match else cItem.get("icon", "")
        )
