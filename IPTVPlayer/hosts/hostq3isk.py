...
class Q3isk(CBaseHostClass):
    ...
    def exploreItems(self, cItem):
        ...
        if main_info_block:
            ...
            # --- Cast (all tax blocks) ---
            all_tax = re.findall(r'<div class="tax">(.*?)</div>', main_info_block, re.S)
            cast_names = []
            for blk in all_tax:
        ...
        for item in items:
            ...
            params = dict(cItem)
            params.update({"title": name, "url": video_url, "desc": info_desc, "type": "video", "category": "video"})
    ...
    def listSeriesUnits(self, cItem):
        ...
        for item in items:
            ...
            params = dict(cItem)
            params.update({"title": "%s%s%s" % (E2ColoR("yellow"), title, E2ColoR("white")), "url": url, "icon": icon, "desc": desc, "category": "list_series_episodes"})
            self.addDir(params)
    ...
    def listSeriesEpisodes(self, cItem):
        ...
        if main_info_block:
            ...
            # --- Add info item ---
            params = dict(cItem)
            params.update({"title": f"{E2ColoR('yellow')}{info_title}{E2ColoR('white')}", "icon": info_icon, "desc": info_desc, "good_for_fav": False})
            self.addDir(params)

        ###################################################
        # EPISODE LIST BLOCK
        ###################################################
        main_block = self.cm.ph.getDataBeetwenMarkers(data, '<div class="BlocksHolder"', '<div style="clear', True)[1]
        ...
        for item in items:
            ...
            params = dict(cItem)
            params.update({"title": colored_title, "url": url, "icon": icon, "category": "explore_item"})
            self.addDir(params)
    ...
    def listMoviesUnits(self, cItem):
        ...
        for item in items:
            ...
            params = dict(cItem)
            params.update({"title": "%s%s%s" % (E2ColoR("yellow"), title, E2ColoR("white")), "url": url, "icon": icon, "desc": desc, "category": "explore_item"})
            self.addDir(params)
