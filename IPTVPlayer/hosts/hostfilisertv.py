...
class FiliserTv(CBaseHostClass):
    ...
    def listItems(self, cItem, nextCategory):
        ...
        if cItem.get('genres', '') not in ['-', '']:
            baseUrl += 'kat={0}&'.format(urllib.parse.quote(cItem['genres']))

        if cItem.get('language', '') not in ['-', '']:
            baseUrl += 'ver={0}&'.format(urllib.parse.quote(cItem['language']))

        if cItem.get('year', '0') not in ['0', '-', '']:
            baseUrl += 'start_year={0}&end_year={1}&'.format(cItem['year'], cItem['year'])

        if cItem.get('sort_by', '0') not in ['0', '-', '']:
            baseUrl += 'sort_by={0}&'.format(urllib.parse.quote(cItem['sort_by']))

        if cItem.get('order', '0') not in ['0', '-', '']:
            baseUrl += 'type={0}&'.format(urllib.parse.quote(cItem['order']))
    ...
    def listSearchResult(self, cItem, searchPattern, searchType):
        ...
        baseUrl = self.getFullUrl('szukaj?q=' + urllib.parse.quote_plus(searchPattern))
    ...
    def getHeaders(self, tries):
        ...
        if self.WaitALittleBit == None:
            try:
                ...
                exec(WaitALittleBit, vGlobals, vLocals)
    ...
    def getVideoLinks(self, videoUrl):
        ...
        if not self.cm.isValidUrl(videoUrl):
            ...
            url = self.getFullUrl('/embed?type=%s&code=%s&code2=%s&salt=%s&title=%s&title2=%s' % (linkParams['type'], linkParams['code'], linkParams['code2'], videoUrl, urllib.parse.quote(linkParams['title1']), urllib.parse.quote(linkParams['title2'])))
