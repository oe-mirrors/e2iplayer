...
#############################################
        ...
        if 'lourdes' == name:
            ...
            if '' == youtube_link:
                youtube_link = self.cm.ph.getSearchGroups(data, '''['"](https://www.youtube.com[^"^']+?)['"?]''')[0]
            if youtube_link:
            ...
            if Url:
               ...
               try:
                  data = self.cm.getURLRequestData(query_data)
                   ...
                   proxy = 'https://proksiak.pl/show.php?u={0}'.format(quote(Url, ''))
                   try:
    ...
    def getResolvedURL(self, url):
        ...
        if 'hdontap' in url:
            ...
            if PY2:
                ...
                ref = base64.b64encode(url.encode()).decode()
            self.defaultParams = {'header': self.HTTP_HEADER, 'use_cookie': True, 'load_cookie': True, 'save_cookie': True, 'cookiefile': COOKIEFILE}
            sts, data = self.getPage(url, 'hdontap.cookie', 'hdontap.com', self.defaultParams)
            if not sts:
                return ''
        ...
        if 'tivix' in url:
            ...
            for x in range(1, 100):
                ...
                if m3u8_url:
                    ...
                    try:
                        if PY2:
                            m3u8_url = base64.b64encode(wynik)
                        else:
                            m3u8_url = base64.b64encode(wynik.encode()).decode()
