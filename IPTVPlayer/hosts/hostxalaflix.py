...
class XalaFlix(CBaseHostClass):
    ...
    def listItems(self, cItem):
        ...
        data = self.cm.ph.getAllItemsBeetwenMarkers(data, 'class="type"', '</a>')
        for item in data:
...
class IPTVHost(CHostBase):
    ...
    def withArticleContent(self, cItem):
        if cItem.get('type', 'video') != 'video' and cItem.get('category', '') != 'list_episodes':
            return False
        return True
