...
from Plugins.Extensions.IPTVPlayer.tools.iptvtools import printDBG, printExc, GetDefaultLang, GetCookieDir
...
class YoutubeIE(object):
	...
	_VALID_URL = (
		...
		r"(?:youtu\.be/|(?:\w+\.)?youtube(?:-nocookie)?\.com/|"
		r"tube\.majestyc\.net/)"  # the various hostnames
		r"(?:.*?\#/)?"
		r"(?:"
		r"(?:(?:v|embed|e)/)"  # v/ or embed/ or e/
		r"|(?:"
		r"(?:watch(?:_popup)?(?:\.php)?)?"  # watch or watch_popup
		r"(?:\?|\#!?)"
		r"(?:.*?&)?"
		r"v="
		r")"
		r")?"
		r")?"  # optional whole group, even naked ID allowed
		r"([0-9A-Za-z_-]+)"  # the video ID
		r"(?(1).+)?"  # anything can follow
		r"$"
	)
	...
	def _extract_yt_initial_variable(self, webpage, regex, video_id, name):
		pattern = (r"%s\s*%s" % (regex, self._YT_INITIAL_BOUNDARY_RE), regex)
		return json_loads(self._search_regex(
			pattern, webpage, name, default="{}"))
	...
	def _real_extract(self, url, allowVP9=False, allowAgeGate=False):
		...
		if len(signatures):
			...
			if not playerUrl:
				for reObj in [r'"assets"\:[^\}]+?"js"\s*:\s*"([^"]+?)"', r'src="([^"]+?)"[^>]+?name="player.*?/base"', '"jsUrl":"([^"]+?)"']:
	...
	def _extract_from_m3u8(self, manifest_url, video_id):
		...
		def _get_urls(_manifest):
			...
			urls = [ls for ls in lines if ls and not ls.startswith('#')]
			return urls
