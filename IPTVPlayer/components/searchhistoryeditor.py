...
class SearchHistoryEditor(Screen):
    ...
    def keyHelp(self):
        ...
        options = [
            GetKeyHelpItem('ok', _("edit the selected entry (same as BLUE) - or drop it while manually reordering"), icon('ok')),
            GetKeyHelpItem('exit', _("close the editor - or stop manual reordering if it's active"), icon('exit')),
            GetKeyHelpItem('red', _("delete the selected entry"), icon('red')),
            GetKeyHelpItem('green', _("enable / disable manual reordering"), icon('green')),
            GetKeyHelpItem('yellow', _("save all changes to disk"), icon('yellow')),
            GetKeyHelpItem('blue', _("edit the selected entry (same as OK)"), icon('blue')),
            GetKeyHelpItem('menu', _("open the options menu - see below"), icon('menu')),
            GetKeyHelpItem('info', _("show this help"), icon('info')),
            GetKeyHelpItem('num', _("jump to entries starting with that letter (if enabled in the settings)"), icon('key_0-9')),
            GetKeyHelpItem('updown', _("move the selection - or move it while manually reordering"), icon('key_updown')),
